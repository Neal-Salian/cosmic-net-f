...
- `virial_penalty = (log r_pruned − log r_full)²` — the **relative, same-graph formulation** (approved Aug 2026), where both ratios use LOOP-FREE physics edges (self-loops excluded; GNN message-passing keeps its loops) and `r_full` is the same graph's action-independent full-graph reference. This supersedes the original `max(0, r−1)²` target-of-1 form: measured on the real dataset, self-loops carried ~100% of PE (ratio ~4e-5 → penalty identically zero), and no available construction lands near ratio 1 (loop-free stellar median 4.4–8.8 with decade-spanning IQR). Scale-invariant to global mass rescaling; label-free; TTA-compatible. Zero-real-edge masks stay finite via a documented eps_ratio=1e-12 log-clamp.
...
- The repo's training-time `VirialLoss` (`model/physics_loss.py`) defines the ratio as `2·KE/(G·M_pred²/R_half)` — a monopole approximation using the **predicted** total mass. The RL reward uses a pairwise `2·KE_retained/(G·Σ_kept M_i·M_j/r_ij)`. Two definitions in one paper need one narrative:
  ...
  2. Report **both** ratios on the full graph in a calibration note. **(CORRECTED Aug 2026, measured on the real dataset: they do NOT agree — pairwise loop-free ~4.4–8.8 median, monopole-style ~8e-4; the original ~10% assumption was empirical wishful thinking. Say so in the paper.)**
  3. **CRITICAL confound:** `PE_retained` shrinks mechanically as edges are dropped (and `KE_retained` via the kept-degree fraction) — both terms track the sparsity *level*, not which edges were kept. A virial ratio in [0.9, 1.1] may be an artifact of pruning to ~50% and would hold for random/degree/distance baselines too. Therefore: **report the virial ratio for EVERY baseline at matched sparsity** (Part 3 table). The RL claim is only "same virial ratio at equal or better accuracy/fidelity" — never "RL achieved virial consistency". **(Resolved Aug 2026: the approved reward now operationalizes this directly — the relative penalty compares each mask to its own graph's full-graph reference, cancelling the shared sparsity-level systematic. Measured (82 real test graphs, radius-2.0): structure-preserving masks ~0.005, random@50% ~2.3, adversarial longest-edge ~7–13.)**
...
| Physics validity | Relative virial penalty (log r_pruned − log r_full)², loop-free; Spearman(ρ, U_ij); Mann-Whitney on kept vs dropped edge distance | relative penalty **reported for every baseline at matched sparsity** — [0.9, 1.1] absolute-ratio band RETIRED Aug 2026 (measured unattainable: no construction lands near 1); ρ ≥ 0.5; kept edges significantly shorter (p < 0.05) |
...
| Method | RMSE (dex) | R² | Scatter (dex) | Keep frac | Fidelity | Spearman ρ vs U_ij | Relative virial penalty (med) | 95% CI coverage |
|---|---|---|---|---|---|---|---|---|
| Full graph (frozen) | 0.1167 | 0.9075 | ~0.10 | 1.0 | 1.0 | — | 0 (reference; measured Aug 2026: abs ratios ~4e-5 w/ loops, 4.4–8.8 loop-free, ~8e-4 monopole) | ~0.95 |
...
| **RL policy (50%)** | **≤0.137** | **≥0.965** | **≤0.10** | **0.5** | **≥0.98** | **≥0.5 (and > mass-ratio row)** | **≤ structured baselines (deg/dist ~0.005 measured); ≪ random (~2.3 measured)** | **≥0.93** |
...
2. **Virial relative penalty is reported for EVERY method at matched sparsity** — because both KE_retained and PE_retained shrink mechanically with sparsity, absolute ratios are not evidence of physical selection. The RL claim is "pruning that does not disturb the graph's own virial structure at equal or better accuracy/fidelity", never "RL achieved virial consistency". (Absolute [0.9, 1.1] band retired Aug 2026 — measured unattainable; see §0.3b.)
...
- [ ] **Two virial definitions**: training-time `VirialLoss` (monopole with predicted mass, model/physics_loss.py) vs the reward's pairwise ratio. Reconcile in one paragraph, report both on the full graph, and report the reward's relative penalty for ALL baselines at matched sparsity — otherwise a reviewer who knows the virial theorem will shred the physics claim. **(Update Aug 2026: the two CANNOT be made to agree numerically — measured medians differ by orders of magnitude (pairwise loop-free 4.4–8.8 vs monopole ~8e-4); the reconciliation paragraph must state they are complementary diagnostics, not equivalent quantities. Also: VirialLoss's R_half input is the TOTAL half-mass radius on the CSV path (verified against groupcat), not the stellar radius its docstring implies.)**
...
- **Sep 2026 notebook rewire (same branch):** notebooks A–D now import the fixed `rls/` package instead of inline copies (deleted `EdgePolicyNet`/`hard_mask`/REINFORCE-loop/TTA-loop reimplementations; `tests/test_notebook_hygiene.py` guards the drift in CI). Clone cells pin + check out `fix/rl-pruning-symmetry`. B calls `train_policy` (headline `topk_scheduled`, penalty as appendix ablation) and val-guarded `fine_tune_gnn`; C aggregates via `build_results_table` and decodes via `eval_mask` with `sparsity_mode` synced from B's provenance; D calls `adapt_at_test_time` with placeholder `tta_kl_coef=0.05`/`tta_lr_decay=0.9` plus a val-subset fail-closed gate cell. CAMELS URL fixed AND verified (live listing + real 14 MB download parsed end-to-end; `set`/`snapshot` config added). STILL NEEDS KAGGLE GPU: the retrain itself, the `tta_kl_coef` sweep, the OOD headline run — static edits only here, no end-to-end run claimed.
- **Sep 2026 audit fixes (`fix/rl-pruning-symmetry` off `feature/rl-pipeline`):** (P0) pairwise-symmetric pruning by construction — output symmetrization (pair-consensus mean probs; input arch unchanged so checkpoints stay valid), `final_symmetric_mask`/`repair_symmetric` with provable `pair_asymmetry_fraction == 0.0` incl. odd/duplicate edges, self-loops always retained; confirmed mechanism first (init 2/10 pairs differ >0.01; ~97/200 pairs asymmetric after thresholding near 0.5). (P0-reward) `rls.sparsity_mode: penalty|topk_scheduled` — top-k pairs at the curriculum target (pair-level selection; directed top-k inflated keep 0.60 vs 0.40), logp re-pointed at the executed mask outside no_grad, `check_curriculum_divergence` guard (|keep-target|>0.1, 5+ epochs past half anneal); `eval_mask` keeps the train/inference decoder matched (verified on real kNN graphs: topk EVAL_keep 0.454 vs target 0.40, asym 0.0; penalty still collapses to 1.0 pre-retune). (P1-StageB) val-guarded `fine_tune_gnn` (best pruned val without full regression beyond `stageb_full_tol`, early-stop, best-restore; returns `(history, info)`), `rls/provenance.py` sha256 backbone guard + `provenance.json`, `stageb_augment: policy|dropout|both`. (P1-TTA) per-step reward decomposition + diagnostic RMSE, `tta_kl_coef` trust region + `tta_lr_decay`, `tta_should_enable` fail-closed val gate. (P1-physics) synthetic 2-body virialized cluster validates G units (ratio==1); near-full-mask test proves ~0 median penalty comes from keep~=1, not eps clamping. (P2) `scripts/multiseed_rl.py` + `summarize_multiseed` bootstrap CIs. 18 new tests; 111 passed, 0 regressions. REMAINING: Stage-A retrain to confirm keep tracks 0.40 + R2 recovery (needs LFS backbone weights / Kaggle GPU); notebook A–D import-from-package dedup (deferred — needs Kaggle execution to verify); CAMELS URL fix + OOD frozen-vs-TTA headline run.
